...
const CourseCard = ({course, isEnterprise, setCourse, setIsAddCourse, setIsUpdate}) => {
    if (!isEnterprise) {
        return <Link to="/course" state={{course: course}} className="shadow-2xl shadow-gray-300 hover:shadow-gray-600 w-fit h-fit min-w-[400px] min-h-[380px] rounded-xl">
        ...
        return <div onClick={() => {
            ...
            <div className="ml-4 mt-4">
                ...
                <div className="">
                    <label className="text-xl text-gray-400">Giáo Viên: {course.teacherName}</label>
                </div>
            </div>
        </div>
    }
